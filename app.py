...
def load_file(path_or_buffer, name=""):
    ...
    if n.endswith(".parquet"):
        ...
        df = pd.read_csv(path_or_buffer, dtype={"county_fips":str})
    if "county_fips" in df.columns:
        df["county_fips"] = (df["county_fips"].fillna("")
                             .astype(str).str.strip()
                             .str.replace(".0","",regex=False)
...
def run_agent(user_message, df, api_key, usda_key):
    ...
    history = [
        ...
        if m["role"] in ("user","assistant") and isinstance(m.get("content"),"str" if True else str)
    ]
    ...
    if has_sims(df):
        sims_note = (
            "\nThis dataset includes simulation scenarios. "
            "For simulation rows, workers_sim=simulated employment, "
            "workers_change=net change, pct_change=% change."
        )

    system = f"""You are an expert on U.S. labor markets and GTAP.
You have access to estimated workers for 65 GTAP sectors across U.S. counties,
years 2021-2024, by skill level and birthplace.{sims_note}

Always use tools for numbers. Use create_map for geographic questions.
Use create_chart for trends. Use query_dataset for specific figures.
For interactive dashboards with dropdowns, tell the user to use the Dashboard tab.

GTAP codes: pdr=Paddy rice, wht=Wheat, gro=Cereal grains, v_f=Vegetables/fruits/nuts,
osd=Oil seeds, ctl=Livestock, rmk=Raw milk, frs=Forestry, cns=Construction,
trd=Trade, hht=Health, edu=Education, obs=Business services."""
...
def render_dashboard(df):
    ...
    # ── Filters ───────────────────────────────────────────────────────────────
    cols = st.columns(5) if _has_sims else st.columns(4)
    ...
    with cols[1] if _has_sims else cols[0]:
    ...
    sector_col = cols[2] if _has_sims else cols[1]
    skill_col  = cols[3] if _has_sims else cols[2]
    bp_col_ui  = cols[4] if _has_sims else cols[3]

    with sector_col:
        secs = ordered_sectors(df)
        slabels = sector_label_map(df)
        sel_sec = st.selectbox("GTAP Sector", ["All sectors"]+secs, key="d_sec",
            format_func=lambda x: slabels.get(x,x) if x!="All sectors" else "All sectors")
    ...
    with bp_col_ui:
        bpc = "birthplace" if "birthplace" in df.columns else "birthplace_label"
    ...
    if sel_sec != "All sectors": filt = filt[filt["gtap_code"]==sel_sec]
    ...
    if sel_scen != "baseline" and _has_sims and "workers_change" in filt.columns:
        ...
        sk_c = int(filt[filt[sk_col]=="Unskilled"]["workers_change"].fillna(0).sum())
        for col, val, lbl, neg in [
            ...
            (k4, f"{sk_c:+,}",   "Unskilled Change",        sk_c<0),
        ]:
            cls = "neg" if neg else ("pos" if val.startswith("+") else "")
    ...
    with mc:
        is_change = (sel_scen != "baseline" and _has_sims and "workers_change" in filt.columns)
        if is_change:
            ...
            cagg = cagg[cagg[wc]>0] if len(cagg) and wc in cagg.columns else cagg
            ccol, cscale, cmid, clbl = wc,"Blues",None,"Workers"
            mtitle = f"Workers by County — {sel_year}"
            if sel_sec != "All sectors": mtitle += f" | {sel_sec}"

        cagg = cagg[cagg["county_fips"].str.len()==5] if len(cagg) else cagg
        ...
        if len(cagg) > 0:
            ...
            fig_map = px.choropleth(
                ...
                title=mtitle, labels={ccol:clbl,"county_name":"County","state":"State"},
                **kw
            )
        ...
        if is_change:
            if sel_sec == "All sectors":
                ...
                fig_bar = px.bar(bd, x="workers_change", y="label", orientation="h",
                    title="Top 15 Sectors by Change",
            ...
            if wc not in filt.columns:
                ...
                bd = (filt.groupby(["gtap_code","gtap_sector"])[wc]
                ...
                fig_bar = px.bar(bd, x=wc, y="label", orientation="h",
                    ...
                    color=wc, color_continuous_scale="Blues",
                    labels={wc:"Workers","label":""})
            else:
                ...
                fig_bar = px.bar(bd, x=wc, y="state", orientation="h",
                    title=f"Top 15 States — {sel_sec} {sel_year}",
                    color=wc, color_continuous_scale="Blues",
                    labels={wc:"Workers","state":"State"})

        fig_bar.update_layout(
            height=430, showlegend=False, plot_bgcolor="white", paper_bgcolor="white",
    ...
    with pc:
        ...
        if is_change:
            ...
            sd = filt.groupby([sk_col,bpc])[wcc].sum().reset_index() if wcc in filt.columns else pd.DataFrame()
            if len(sd):
                sd["group"] = sd[sk_col] + " / " + sd[bpc]
...
with tab2:
    bpc2 = "birthplace" if "birthplace" in df.columns else "birthplace_label"
