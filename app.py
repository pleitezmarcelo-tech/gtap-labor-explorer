...
GTAP_TO_MODEL = {
    ...
    "mvh":("Autos","Autos"),"nmm":("Cment","Cement"),"p_c":("Petro","Petroleum"),
    "chm":("Chem","Chemicals"),"bph":("Pharma","Pharmaceuticals"),
...
st.set_page_config(page_title="GTAP Labor Data Explorer",page_icon="?",layout="wide",initial_sidebar_state="expanded")
st.markdown("""<style>
...
def load_parquet():
    ...
    return _prep(pd.read_parquet(path))
...
def _prep(df):
    if "county_fips" in df.columns:
        df["county_fips"]=(df["county_fips"].fillna("").astype(str).str.strip()
...
def run_agent(msg,df,key,ukey):
    ...
    hist=[{"role":m["role"],"content":m["content"]} for m in st.session_state.messages[-12:]
          if m["role"] in ("user","assistant") and isinstance(m.get("content"),str)]
    hist.append({"role":"user","content":msg})
    sn="\nDataset has simulation scenarios. workers_sim=simulated, workers_change=net change." if has_sims(df) else ""
    sys=f"You are an expert on U.S. labor markets and GTAP.{sn}\nUse tools for numbers. create_map for maps. create_chart for trends."
    figs,text,msgs=[],"",[*hist]
    for i in range(6):
        r=client.messages.create(model="claude-sonnet-4-20250514",max_tokens=2048,system=sys,tools=TOOL_DEFINITIONS,messages=msgs)
        for b in r.content:
        ...
        for b in r.content:
            if b.type=="tool_use":
                ...
                if isf and res is not None:
                    fid=f"fig_{i}_{b.id[:8]}"; figs.append((fid,res)); ct=json.dumps({"success":True,"figure_id":fid})
                else: ct=json.dumps(res)
                tr.append({"type":"tool_result","tool_use_id":b.id,"content":ct})
        msgs.append({"role":"assistant","content":r.content}); msgs.append({"role":"user","content":tr})
    return text,figs
...
def render_dashboard(df):
    ...
    agg=st.radio("Sector view",["65 GTAP sectors","27 Model sectors"],horizontal=True,key="d_agg")
    use_agg=(agg=="27 Model sectors")
    sc="_msc" if use_agg else "gtap_code"
    sd="_msd" if use_agg else "gtap_sector"
    st2="Model Sectors" if use_agg else "GTAP Sectors"
    nc=5 if hs else 4; cols=st.columns(nc)
    with cols[0]:
        if hs:
            so=sorted(df["scenario"].unique().tolist())
            ss=st.selectbox("Scenario",so,key="d_scen",format_func=lambda x:SMETA.get(x,{}).get("label",x))
        else: ss="baseline"
    ...
    with (cols[2] if hs else cols[1]):
        if use_agg:
            msecs=sorted([s for s in df[sc].dropna().unique().tolist() if s])
            mlmap={r[sc]:f'{r[sc]} -- {r[sd]}' for _,r in df[[sc,sd]].drop_duplicates().iterrows() if pd.notna(r[sc]) and r[sc]}
    ...
    col=SMETA.get(ss,{}).get("color","#2E5496"); ic=(ss!="baseline" and hs and "workers_change" in filt.columns)
    if hs and ss!="baseline":
        lbl=SMETA.get(ss,{}).get("label",ss)
        st.markdown(f'<div style="background:{col}18;border-left:4px solid {col};padding:8px 14px;border-radius:6px;margin-bottom:10px;"><b style="color:{col}">{lbl}</b></div>',unsafe_allow_html=True)
    ...
    with mc2:
        if ic:
            ...
            cc,cs,cm,cl=wc,"Blues",None,"Workers"; mt=f"Workers by County -- {sy}"+(f" | {sel_sec}" if sel_sec!="All sectors" else "")
    ...
    with bc2:
        if ic:
            if sel_sec=="All sectors":
                ...
                fig=px.bar(bd,x="workers_change",y="label",orientation="h",title=f"Top 15 {st2} by Change",
                    color="workers_change",color_continuous_scale="RdYlGn",color_continuous_midpoint=0,labels={"workers_change":"Change","label":""})
            else:
                bd=filt.groupby("state")["workers_change"].sum().reset_index().sort_values("workers_change").head(15)
                fig=px.bar(bd,x="workers_change",y="state",orientation="h",title=f"Top 15 States -- {sel_sec}",
                    color="workers_change",color_continuous_scale="RdYlGn",color_continuous_midpoint=0,labels={"workers_change":"Change","state":"State"})
        else:
            if wc not in filt.columns: fig=px.bar(title="No data")
                ...
                bd["label"]=bd[sc].astype(str)+" -- "+bd[sd].astype(str).str[:20]
                fig=px.bar(bd,x=wc,y="label",orientation="h",title=f"Top 15 {st2} -- {sy}",
                    color=wc,color_continuous_scale="Blues",labels={wc:"Workers","label":""})
            else:
                bd=filt.groupby("state")[wc].sum().reset_index().sort_values(wc,ascending=False).head(15)
                fig=px.bar(bd,x=wc,y="state",orientation="h",title=f"Top 15 States -- {sel_sec} {sy}",
                    color=wc,color_continuous_scale="Blues",labels={wc:"Workers","state":"State"})
    ...
    with tc:
        ...
        fig=px.line(td,x="year",y="workers_base",title="Baseline Trend 2021-2024",markers=True,
            color_discrete_sequence=["#2E5496"],labels={"workers_base":"Workers","year":"Year"})
        if ss!="baseline" and hs and "workers_sim" in filt.columns:
            sv=filt["workers_sim"].fillna(0).sum()
            fig.add_scatter(x=[2024],y=[sv],mode="markers",marker=dict(size=14,color=col,symbol="diamond"),
    ...
    with pc:
        if ic:
            ...
            fig=px.bar(sd2.sort_values("workers_change"),x="workers_change",y="group",orientation="h",
                title="Change by Group",color="workers_change",color_continuous_scale="RdYlGn",
                color_continuous_midpoint=0,labels={"workers_change":"Change","group":""})
            fig.update_layout(height=280,plot_bgcolor="white",paper_bgcolor="white",coloraxis_showscale=False,
                xaxis=dict(tickformat=",d",gridcolor="#f0f0f0"),title_font_size=14,margin=dict(t=40,b=10,l=10,r=10))
        else:
            wcc=wc if wc in filt.columns else "workers_base"
            if wcc in filt.columns and len(filt):
                sd2=filt.groupby([sk,bp])[wcc].sum().reset_index(); sd2["group"]=sd2[sk]+" / "+sd2[bp]
                fig=px.pie(sd2,values=wcc,names="group",title=f"Skill x Birthplace -- {sy}",
    ...
    if hs:
        st.markdown("---"); st.markdown("#### Scenario Comparison -- 2024")
        comp=[]
...
with st.sidebar:
    ...
    if st.session_state.df is not None:
        ...
        st.markdown('<div class="ss">',unsafe_allow_html=True); st.markdown("**Summary**")
        try:
...
if st.session_state.df is None: st.info("Click **Load Data** in the sidebar to get started."); st.stop()
df=st.session_state.df
tab1,tab2=st.tabs(["Dashboard","Ask Claude"])
with tab1: render_dashboard(df)
with tab2:
    bp2=bpcol(df); latest=int(df["year"].max()) if "year" in df.columns else 2024
    ...
    for i,msg in enumerate(st.session_state.messages):
        with st.chat_message(msg["role"]):
            ...
            for fid in msg.get("figures",[]):
                if fid in st.session_state.figures: st.plotly_chart(st.session_state.figures[fid],key=f"h_{fid}_{i}")
